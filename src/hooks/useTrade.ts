import { Currency, CurrencyAmount, Pair, Trade } from '@levinswap/uniswap-sdk';
...
export function useTradeExactIn(currencyAmountIn?: CurrencyAmount, currencyOut?: Currency): Trade | null {
  ...
  const [tokenA, tokenB] = chainId
    ? [wrappedCurrency(currencyAmountIn?.currency, chainId), wrappedCurrency(currencyOut, chainId)]
    : [undefined, undefined];

  const basePairs = useMemo(
    () => bases.flatMap(base => bases.map(otherBase => [base, otherBase]))
